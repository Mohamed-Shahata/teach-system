 ...
 */
export const uploadTargetSchema = z.enum(["course-thumbnail", "lesson-file", "lesson-video", "avatar"]);
...
export const signUploadSchema = z.object({
  ...
  // Required for `target: "lesson-file"` and `target: "lesson-video"` —
  // the lesson must already exist (a file/video is always attached to
  // an existing lesson, unlike a course thumbnail which may precede the
  // course itself), and its own `courseId` is used to build the folder
  // rather than trusting a client-supplied one. See
  // docs/cloudinary/README.md "Folder structure".
