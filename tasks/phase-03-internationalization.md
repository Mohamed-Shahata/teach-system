...
- Description: Configure `[locale]` segment, `proxy.ts` locale resolution, `i18n/config.ts`, base `messages/en.json` + `messages/ar.json`.
- Goal: `/en/*` and `/ar/*` render with correct `lang`/`dir`.
- Dependencies: TASK-102
- Affected modules: `proxy.ts`, `i18n/*`, `app/[locale]/layout.tsx`
