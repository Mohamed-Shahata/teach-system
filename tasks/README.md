...
| 36 | Caching Strategy & Performance | Done |
| 15 | Security | Done |
| 16 | Testing | Done |
| 17 | Deployment | Not Started |
| 18 | MVP Finalization | Done |
...
> TASK-3603 (Firestore read reduction pass on the newest features) is
> now `Done`: fixed the one concrete N+1 TASK-3601 flagged —
> `studentService.getCourseStudentsProgress` was calling
> `lessonProgressRepository.listByStudentForLessons` once per enrolled
> student. New `lessonProgressRepository.listByStudentsForLessons`
> batches the full student × lesson set into a single `getAll`; the
> service groups the flat result back by student itself. Behavior/
> return shape unchanged. See `phase-36-caching-and-performance.md`'s
> TASK-3603 note for the full detail, including why the other two
> TASK-3601 findings were left out of this task's scope. Phase 36 stays
> `In Progress`: TASK-3602 (server-side caching) and TASK-3604
> (client-fetch audit) remain `Not Started`, both unblocked.

> TASK-3602 (server-side caching for `subjects`/`educationStages`) is
> now `Done`: both repositories' `list()` calls are now cached
> in-process (5-minute TTL, invalidated on write) via new
> `lib/server/cache/memoryCache.ts` — fixes TASK-3601's "re-read fresh
> from at least seven services" finding at the source. The teacher
> directory and Phase 33 analytics aggregations were left uncached this
> pass (need a keyed, not flat, cache) — see
> `phase-36-caching-and-performance.md`'s TASK-3602 note. Also fixed in
> passing: `docs/architecture/performance-audit.md` had been written to
> a stray root-level `architecture/` folder instead of `docs/
> architecture/` by a prior session — moved to the correct path.
> TASK-3604 (client-fetch audit) is next; both this and TASK-3603 are
> now `Done`.

> TASK-3604 (client-fetch audit) is now `Done`, closing **Phase 36**:
> audited every dashboard/analytics page for redundant client-side
> fetches and found none — the project's existing server-fetch-then-
> pass-as-prop pattern already prevents the duplication this task
> looked for, so no code change was needed. See
> `phase-36-caching-and-performance.md`'s TASK-3604 note. All four of
> this phase's tasks (TASK-3601–3604) are now `Done`. Next: Phase 11
> (Enrollment, blocked on TASK-1105's payment-gateway decision) or
> Phase 15/16 (`In Progress`, both blocked on real network/emulator
> access this sandbox doesn't have) — no fully unblocked phase remains
> untouched.

> **Housekeeping, this session:** found the same drift pattern already
> fixed once for `architecture/` vs `docs/architecture/` — a prior
> session had started writing this file and Phase 18/36 to a root-level
> `tasks/` folder while every other phase file (01–17, 19–35) still only
> existed at the original `docs/tasks/` path, silently forking the task
> tracker in two. Consolidated: every phase file missing from `tasks/`
> was copied over from `docs/tasks/`, `docs/tasks/` was removed, and
> `README.md`'s link to the task breakdown was repointed to `tasks/
> README.md`. No task content changed, only the location.
>
> With that fixed, the actual next-task check: Phase 11 remains blocked
> (TASK-1105 needs a payment-gateway decision), Phase 15/16 remain
> blocked (both need real network/Firestore-emulator access this
> sandbox doesn't have), and Phase 17 (Deployment) — TASK-1701–1703 —
> all three require a live Vercel/Firebase deployment with real
> credentials, which is equally unavailable here. No phase is currently
> actionable in this sandbox without one of those three externally-
> blocked prerequisites.

> TASK-1503 (Security review pass) is now `Done`, closing **Phase 15**:
> found the phase's own "Blocked" note was stale (TASK-603 was already
> `Done`; only TASK-1105's payment-gateway ADR remains genuinely open
> project-wide, treated as a documented known gap rather than a
> blocker). Walked `security/README.md`'s threat-model table row by row
> against the implementation — every mitigation held up; two doc-
> accuracy fixes made (a nonexistent `CourseAccessService` name
> corrected to the real `assertStudentHasCourseAccess` guard; the
> session row's "revocation on logout/password change" corrected to
> "on logout" since password changes go through Firebase's reset-link
> flow, not an in-session change). No architectural changes resulted,
> so nothing new under `decisions/`. See `phase-15-security.md`'s
> TASK-1503 note for the full row-by-row trail.

> TASK-1604 (i18n/RTL/theme regression checklist) is now `Done`,
> closing **Phase 16**: the three automated checks were already clean;
> the two items needing a real browser (responsive, focus-visible) got
> a source-level audit instead. Found and fixed six real focus-visible
> gaps (sidebar nav links, the sidebar collapse toggle, `DropdownMenu`'s
> shared trigger button, both `Pagination` buttons, `Breadcrumb`'s
> links, three clickable-notification-row components, two upload
> dropzones) and confirmed the two systemic responsive mechanisms
> (collapsing sidebar, `overflow-x-auto` tables) are in place
> everywhere. Pixel-level confirmation still needs a real browser —
> flagged as a residual limitation, not silently assumed — but every
> structural gap findable from source is fixed. See
> `phase-16-testing.md`'s TASK-1604 note for the full list. All four of
> this phase's tasks (TASK-1601–1604) are now `Done`.
...
> TASK-1803 (Future roadmap review) is now `Done`, closing **Phase 18**:
> a documentation-only extensibility audit of `ownership-model.md`,
> `course.enrollmentType`, and `question.type`. The first two hold up
> (proven in practice by post-MVP phases); `question.type` does not —
> both existing types share one options-based model, so a free-response
> question type would need new schema/grading work, not an enum
> addition. See `docs/architecture/future-roadmap.md`'s new
> "Extensibility review (TASK-1803)" section for the full findings.
> Next: Phase 11 (Enrollment, blocked on TASK-1105's payment-gateway
> decision), Phase 15/16 (`In Progress`, both blocked on real
> network/emulator access this sandbox doesn't have), or Phase 36
> (Caching/Performance, `Not Started`) — Phase 36 is the only fully
> unblocked phase left.

> TASK-3601 (Performance audit) is now `Done`, moving Phase 36 to
> `In Progress`: a static-code-review findings doc (no network/emulator
> in this sandbox — traced from source, not measured live). Top finding:
> a genuine N+1 in `studentService.getCourseStudentsProgress` (one
> per-enrolled-student query instead of one batched read); also flagged
> uncached `subjects`/`educationStages` reads across seven services and
> two unbounded full-collection scans for small "recent" lists. Phase
> 33's analytics aggregations — this task's named suspect — turned out
> already well-batched. See `docs/architecture/performance-audit.md` for
> the full ranked list. TASK-3602 (caching), TASK-3603 (fix the N+1 +
> anything else flagged), and TASK-3604 (client-fetch audit) are all
> next — each only depended on this task.
