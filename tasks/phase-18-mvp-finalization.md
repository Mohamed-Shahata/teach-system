...
> TASK-1803 is now `Done`, closing **Phase 18** (all three tasks —
> TASK-1801–1803 — are now `Done`): a point-in-time extensibility
> review, not code changes. Two of the three claims checked out —
> `architecture/ownership-model.md`'s "adding a new collection"
> checklist has already been exercised by every post-MVP collection
> with no changes needed, and `course.enrollmentType` was proven
> extensible in practice when Phase 29's subscriptions landed as a
> parallel access path rather than a third enum value. The third,
> `question.type`, does **not** hold up — both existing types share one
> options/`correctOptionIds` model end to end, and a genuinely
> open-ended type (short answer/essay) would need new schema fields and
> a real grading-path decision, not an enum addition; flagged rather
> than glossed over. Full findings in
> `docs/architecture/future-roadmap.md`'s new "Extensibility review
> (TASK-1803)" section. No code/test changes — documentation only.
