...
import type { AdminProfile } from "@/lib/server/services/adminSettingsService";
...
/**
 * TASK-1907 — Admin's own account settings: display name (editable),
 * password (via a generated one-time reset link, per ADR 0005 — this app
 * never accepts a raw new password over the wire), and profile picture
 * (Phase 5). Mirrors `TeacherSettingsForm` / `StudentSettingsForm`.
 */
export function AdminSettingsForm({ initialProfile }: AdminSettingsFormProps) {
  ...
  async function onSubmitName(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const body = (await res.json()) as { profile: AdminProfile };
  ...
  async function onGenerateResetLink() {
    ...
    try {
      const res = await fetch("/api/admin/settings/password-reset-link", { method: "POST" });
