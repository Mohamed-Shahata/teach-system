...
/**
 * TASK-705 — Teacher's own account settings: display name, password
 * (one-time reset link, ADR 0005), and profile picture. Mirrors
 * `StudentSettingsForm` (TASK-1005) / `AdminSettingsForm` (TASK-1907).
 */
export function TeacherSettingsForm({ initialProfile }: TeacherSettingsFormProps) {
  ...
  async function onGenerateResetLink() {
    ...
    try {
      const res = await fetch("/api/teacher/settings/password-reset-link", { method: "POST" });
  ...
  async function onAvatarSelected(event: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      const res = await fetch("/api/teacher/settings/avatar", {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
