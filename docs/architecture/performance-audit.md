...
## TASK-3603 update — N+1 fix

`studentService.getCourseStudentsProgress` (finding above) has been
fixed: `lessonProgressRepository.listByStudentsForLessons` now batches
one `getAll` across every enrolled student's lesson-progress docs,
replacing the old one-`getAll`-per-student loop. See
`tasks/phase-36-caching-and-performance.md`'s TASK-3603 note for the
full detail.

## TASK-3602 update — caching added

`subjectRepository.list()` and `educationStageRepository.list()` are
now cached (`lib/server/cache/memoryCache.ts`, a 5-minute per-process
TTL cache, invalidated on any `create`/`update`/`delete` to either
collection) — this fixes the "re-read fresh from at least seven
services" finding above at the source, since every caller already
funnels through these two `list()` methods. The public teacher
directory and Phase 33's analytics aggregations (this task's other two
named candidates) were left uncached this pass — both need a properly
keyed cache (per-teacher / per-filter-range) rather than this single-
slot one, since a flat TTL cache would either serve one teacher's data
for another's request or ignore the `granularity`/date-range parameter
entirely. See `tasks/phase-36-caching-and-performance.md`'s TASK-3602
note for the full reasoning.

## TASK-3604 — client-side data fetching review

Audited every dashboard/analytics page (Admin/Teacher/Student) and
every client component mounted on them for redundant same-page fetches
of the same data.

**Finding: no redundant client-side fetches found.** The project's
existing pattern already prevents the failure mode this task looked
for:

- Every dashboard `page.tsx` is a server component. All page-level data
  (`systemStats`, profile, schedule, notifications, invoices, etc.) is
  fetched once server-side and passed down as `initial*` props — client
  components hydrate from that prop instead of re-fetching it on mount.
- Client components that do poll on an interval
  (`AuditNotificationsPanel` → `/api/notifications/mine`,
  `ClassReminderBanner` → `/api/teacher/notifications`,
  `MeetingNotifications` → `/api/student/notifications`) each own a
  distinct endpoint and a distinct concern, even when more than one is
  mounted on the same dashboard page (e.g. `teacher/dashboard` mounts
  both `ClassReminderBanner` and `AuditNotificationsPanel`) — neither
  fetches data the other already has.
- Grepping every client-side `fetch(...)` call project-wide for
  same-page duplication turned up no endpoint hit by more than one
  component mounted together; the handful of endpoints called more
  than once in the codebase (`/api/teacher/schedule`,
  `/api/teacher/profile`, `/api/admin/subjects`, etc.) are all repeat
  calls *within a single component* for its own CRUD operations (GET
  on load, PATCH/POST/DELETE on user action), not duplicate GETs across
  components.

No shared query/cache layer (SWR/React Query) exists in this codebase,
but none was needed to satisfy this task's acceptance criteria — there
is no duplication to consolidate today. Worth revisiting if a future
page mounts two components that both need the same list (e.g. two
widgets both needing `subjects`), at which point lifting the fetch to
the shared parent (already the project's own stated fallback for this
case) is the right fix rather than introducing a new dependency.
