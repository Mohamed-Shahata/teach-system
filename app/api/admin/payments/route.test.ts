...
vi.mock("@/lib/server/services/adminPaymentsService", () => ({
  adminPaymentsOverviewService: { listAll },
}));
...
describe("GET /api/admin/payments", () => {
  ...
  it("lists all payments", async () => {
    ...
    listAll.mockResolvedValue([{ id: "p1" }]);

    const res = await GET(new Request("http://localhost/api/admin/payments"));

    expect(res.status).toBe(200);
    expect(listAll).toHaveBeenCalledWith({ uid: "admin-1", email: "a@b.com", role: "admin" }, undefined);
  });
  ...
  it("passes the status query param through", async () => {
    ...
    expect(listAll).toHaveBeenCalledWith({ uid: "admin-1", email: "a@b.com", role: "admin" }, "pending");
  });
  ...
  it("returns 403 for a non-admin session", async () => {
    ...
    listAll.mockRejectedValue(new ForbiddenError());
