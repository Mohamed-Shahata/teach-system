...
vi.mock("@/lib/server/services/adminSettingsService", () => ({
  adminSettingsService: { getProfile, updateDisplayName },
}));
...
describe("GET /api/admin/settings", () => {
  ...
  it("returns the admin's profile", async () => {
    requireSession.mockResolvedValue({ uid: "admin-1", email: "a@b.com", role: "admin" });
    getProfile.mockResolvedValue({ uid: "admin-1", email: "a@b.com", displayName: "Layla" });

    const res = await GET();

    expect(res.status).toBe(200);
    expect(getProfile).toHaveBeenCalledWith({ uid: "admin-1", email: "a@b.com", role: "admin" });
  });
...
describe("PATCH /api/admin/settings", () => {
  ...
  it("updates the display name", async () => {
    requireSession.mockResolvedValue({ uid: "admin-1", email: "a@b.com", role: "admin" });
    updateDisplayName.mockResolvedValue({ uid: "admin-1", email: "a@b.com", displayName: "New Name" });

    const res = await PATCH(makeRequest({ displayName: "New Name" }));

    expect(res.status).toBe(200);
    expect(updateDisplayName).toHaveBeenCalledWith({ uid: "admin-1", email: "a@b.com", role: "admin" }, "New Name");
  });
